...
to encapsulate the configurations options available for each plugin as currently documented in channeld's --help command.

It makes use of the compiled channeld program and runs the --help command, capturing the output.
...
The script then uses the parsed output to generate *PluginArgs.py scripts, placing them in the ChanneldPluginArgs directory.
...
Each *PluginArgs.py file contains one dataclass that captures the available configuration options for that plugin via channeld command line.

Each config option is represented by 3 member variables, for example:
1) blocksDir: str=None
    --This is the field that will be populated when the dataclass is used by other scripts to configure channeld
2) _blocksDirChanneldDefault: str='"blocks"'
    --This field captures the default value in the channeld output.  This will be compared against the first field to see if the configuration
    option will be required on the command line to override the default value when running channeld.
3) _blocksDirChanneldArg: str="--blocks-dir"
...
1) supportedChanneldArgs
    -- Provides a list of all the command line config options currently supported by the dataclass
2) __str__
    -- Provides the command line argument string for the current configuration to pass to channeld
...
- There are some hardcoded edge cases when trying to determine the types associated with certain default argument parameters.
  These may need to be updated to account for new/different options as they are added/removed/modified by channeld

Note:
- To help with maintainability the validate_channeld_plugin_args.py test script is provided which validates the current
  *PluginArgs dataclass configuration objects against the current channeld --help output to notify developers when
  configuration options have changed and updates are required.
"""


def main():
    result = subprocess.run(["../../../bin/channeld", "--help"], stdout=subprocess.PIPE, universal_newlines=True)
    ...
    def writeDataclass(plugin:str, dataFieldDict:dict, pluginOptsDict:dict):
        ...
        with open(pluginArgsFile, 'w') as dataclassFile:
            ...
            dataclassFile.write(f"This file/class was generated by generate_channeld_plugin_args_class_files.py\n")
            dataclassFile.write(f"\"\"\"\n\n")
            ...
            for key, value in pluginOptsDict[plugin].items():
                ...
                if match is not None:
                    ...
                    try:
                        ...
                        dataclassFile.write(f"    _{newKey}ChanneldDefault: int={numVal}\n")
                        dataclassFile.write(f"    _{newKey}ChanneldArg: str=\"{key}\"\n")
                    except ValueError:
                        strValue = str(value)
                        quote = "\'" if re.search("\"", strValue) else "\""
                        dataclassFile.write(f"    {newKey}: str=None\n")
                        dataclassFile.write(f"    _{newKey}ChanneldDefault: str={quote}{strValue}{quote}\n")
                        dataclassFile.write(f"    _{newKey}ChanneldArg: str=\"{key}\"\n")
                else:
                    if re.search("deepmind", newKey, re.IGNORECASE) or \
                       ...
                       re.search("disable", newKey, re.IGNORECASE):
                        ...
                        dataclassFile.write(f"    _{newKey}ChanneldDefault: bool=False\n")
                        dataclassFile.write(f"    _{newKey}ChanneldArg: str=\"{key}\"\n")
                        ...
                        dataclassFile.write(f"    {newKey}: int=None\n")
                        dataclassFile.write(f"    _{newKey}ChanneldDefault: int=None\n")
                        dataclassFile.write(f"    _{newKey}ChanneldArg: str=\"{key}\"\n")
                    else:
                        dataclassFile.write(f"    {newKey}: str=None\n")
                        dataclassFile.write(f"    _{newKey}ChanneldDefault: str=None\n")
                        dataclassFile.write(f"    _{newKey}ChanneldArg: str=\"{key}\"\n")
...
def main():\n\
    ...
    print(pluginArgs.supportedChanneldArgs())\n\
    exit(0)\n\n\
