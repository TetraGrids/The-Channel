...
public:
    ...
    asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( "ra.token"_n, act, "accounts"_n, name(symbol(CORE_SYMBOL).to_symbol_code().value) );
    ...
    void setup_system_accounts(){
       ...
       {
           const auto& accnt = control->db().get<account_object,by_name>( "ra.token"_n );
           abi_def abi;
       ...
       BOOST_CHECK_EQUAL( core_from_string("1000000000.0000"), get_balance( name("ra") ) );
