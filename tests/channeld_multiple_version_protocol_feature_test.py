...
###############################################################
# channeld_multiple_version_protocol_feature_test
#
# Test for verifying that older versions of channeld can work with newer versions of channeld.
#
###############################################################
...
def restartNode(node: Node, chainArg=None, addSwapFlags=None, channeldPath=None):
    if not node.killed:
        node.kill(signal.SIGTERM)
    isRelaunchSuccess = node.relaunch(chainArg, addSwapFlags=addSwapFlags,
                                      timeout=5, channeldPath=channeldPath)
...
try:
    ...
    assert cluster.launch(pnodes=4, totalNodes=4, prodCount=1, totalProducers=4,
                          extraChanneldArgs=" --plugin eosio::producer_api_plugin ",
    ...
    # Finally, when we restart the 4th node with the version of channeld that supports protocol feature,
    ...
    oldNode.relaunch(chainArg="--import-reversible-blocks {}".format(portableRevBlkPath), timeout=1, channeldPath="programs/channeld/channeld")
    os.remove(portableRevBlkPath)

    restartNode(oldNode, chainArg="--replay", channeldPath="programs/channeld/channeld")
