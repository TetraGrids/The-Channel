...
###############################################################
# channeld_under_min_avail_ram
#
# Sets up 4 producing nodes using --chain-state-db-guard-size-mb and --chain-state-db-size-mb to verify that channeld will
...
try:
    ...
    maxRAMValue=1010
    extraChanneldArgs=" %s %d %s %d  --http-max-response-time-ms 990000 " % (minRAMFlag, minRAMValue, maxRAMFlag, maxRAMValue)
    # test relies on production continuing on restart
    extraChanneldArgs+=" --production-pause-vote-timeout-ms 0 "
    if cluster.launch(onlyBios=False, pnodes=pNodes, totalNodes=totalNodes, totalProducers=totalNodes, activateIF=activateIF, extraChanneldArgs=extraChanneldArgs) is False:
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    timeOutCount+=1
                    if timeOutCount>=3:
                        Print("Failed to push create action to eosio contract for %d consecutive times, looks like channeld already exited." % (timeOutCount))
    ...
    # min and max are subjective, just assigned to make sure that many small changes in channeld don't 
