...
issue_tokens( validating_tester& t, account_name issuer, account_name to, const asset& amount,
              std::string memo = "", account_name token_contract = "ra.token"_n )
{
...
BOOST_FIXTURE_TEST_CASE( get_scope_test, validating_tester ) try {
   ...
   create_accounts({ "ra.token"_n, "ra.ram"_n, "ra.ramfee"_n, "ra.stake"_n,
      "ra.bpay"_n, "ra.vpay"_n, "ra.saving"_n, "ra.names"_n });
   ...
   push_action("ra.token"_n, "create"_n, "ra.token"_n, act );
   ...
   eosio::chain_apis::read_only::get_table_by_scope_params param{"ra.token"_n, "accounts"_n, "inita", "", 10};
...
BOOST_FIXTURE_TEST_CASE( get_table_test, validating_tester ) try {
   ...
   if (result.rows.size() >= 4u) {
      ...
      BOOST_REQUIRE_EQUAL("ra", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("ra", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("ra", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("ra", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, validating_tester ) try {
   ...
   auto bidname = [this]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return push_action( "ra"_n, "bidname"_n, bidder, fc::mutable_variant_object()
                          ("bidder",  bidder)
                          ("newname", newname)
                          ("bid", bid)
                          );
