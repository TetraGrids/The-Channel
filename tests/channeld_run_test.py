...
# channeld_run_test
#
# General test that tests a wide range of general use actions around channeld and keyd
#
###############################################################
...
errFileName=f"{cluster.channeldLogPath}/node_00/stderr.txt"
...
try:
    ...
    if localTest and not dontLaunch:
        ...
        abs_path = os.path.abspath(os.getcwd() + '/unittests/contracts/ra.token/ra.token.abi')
        traceChanneldArgs=" --http-max-response-time-ms 990000 --trace-rpc-abi ra.token=" + abs_path
        extraChanneldArgs=traceChanneldArgs + " --plugin eosio::prometheus_plugin --database-map-mode mapped_private "
        specificChanneldInstances={0: "bin/channeld"}
        if cluster.launch(totalNodes=2, prodCount=prodCount, activateIF=activateIF, onlyBios=onlyBios, dontBootstrap=dontBootstrap, extraChanneldArgs=extraChanneldArgs, specificChanneldInstances=specificChanneldInstances) is False:
            cmdError("launcher")
    ...
    contractDir="unittests/contracts/ra.token"
    wasmFile="ra.token.wasm"
    abiFile="ra.token.abi"
    ...
    node.processChanCmd(signCmd, "Sign and push a transaction", False, True)
    ...
    buyRamAmount = '"85.5 A"'
    buyRamCmd = f"--abi-file core.vaulta:unittests/contracts/ra.system/ra.system.abi system buyram -j {testeraAccount.name} {testeraAccount.name} {buyRamAmount} -p {testeraAccount.name}@active"
    trace=node.processChanCmd(buyRamCmd, "buy ram from core.vaulta", silentErrors=False, exitOnError=True)
    ...
    # the resulting exception in http-plugin is caught and doesn't cause channeld to crash (leap issue #1372).
    contractDir="unittests/contracts/ra.token"
    wasmFile="ra.token.wasm"
    abiFile="ra.token.bad.abi"
    Print("Publish contract")
    trans=node.publishContract(currencyAccount, contractDir, wasmFile, abiFile, waitForTransBlock=True)
    if trans is None:
        cmdError("%s set contract currency1111" % (ClientName))
    ...
    # because we set a bad abi (missing type, see "ra.token.bad.abi") on the contract, the
