...
###############################################################
# channeld_signal_throw_test
#
# Verify throwing in signal handlers is handled correctly by channeld.
#
# A controller_emit_signal_exception should cause channeld to gracefully shutdown.
# Any other exception type should be ignored and channeld continue running.
#
###############################################################
...
try:
    ...
    extraChanneldArgs="--plugin eosio::test_control_api_plugin"
    specificExtraChanneldArgs= {"0": "--enable-stale-production"}
    if cluster.launch(pnodes=pnodes, totalNodes=total_nodes, totalProducers=pnodes, delay=delay,
                      activateIF=True, extraChanneldArgs=extraChanneldArgs, specificExtraChanneldArgs=specificExtraChanneldArgs) is False:
        errorExit("Failed to stand up eos cluster.")

    assert cluster.biosNode.getInfo(exitOnError=True)["head_block_producer"] != "ra", "launch should have waited for production to change"
