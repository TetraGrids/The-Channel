## Command
```sh
chan net disconnect [OPTIONS] host
```
...
## Command Usage
The following information shows the different positionals and options you can use with the `chan net disconnect` command:
...
[[info | Note]]
| `chan` is bundled with the Antelope software. [Installing Antelope](../../../00_install/index.md) will also install the `chan` and `keyd` command line tools.
* You have access to a producing node instance with the [`net_api_plugin`](../../../01_channeld/03_plugins/net_api_plugin/index.md) loaded.

## Examples
The following examples demonstrate how to use the `chan net disconnect` command:
...
```sh
chan net disconnect localhost:9002
```
**Output:**
```console
"connection removed"
...
```sh
chan -u http://127.0.0.1:8001 net disconnect localhost:9002
```
**Output:**
```console
"connection removed"
...
**Note:** If any of the above commands are re-executed, `chan` returns the following message as expected:  
```console
