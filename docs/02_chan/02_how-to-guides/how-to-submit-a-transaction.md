...
[[info]]
| Be aware that if a clear text `data` field is used, chan need to fetch copies of required ABIs using `channeld` API. That operation has a performance overhead on `channeld`
...
```sh
chan push transaction TRX_FILE.json
```

* Submit a transaction from a JSON:

```sh
chan push transaction JSON
```
