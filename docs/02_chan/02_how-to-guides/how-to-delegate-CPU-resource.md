...
* Familiarize with the [`chan system delegatebw`](../03_command-reference/system/system-delegatebw.md) command and its parameters.
...
```sh
chan system delegatebw <from> <receiver> <stake_net_quantity> <stake_cpu_quantity>
```
...
```sh
chan system delegatebw bob alice "0 SYS" "0.01 SYS"
```
```json
...
#   ra.token <= ra.token::transfer        {"from":"bob","to":"ra.stake","quantity":"0.0010 SYS","memo":"stake bandwidth"}
#  bob <= ra.token::transfer        {"from":"bob","to":"ra.stake","quantity":"0.0010 SYS","memo":"stake bandwidth"}
#   ra.stake <= ra.token::transfer        {"from":"bob","to":"ra.stake","quantity":"0.0010 SYS","memo":"stake bandwidth"}
```
