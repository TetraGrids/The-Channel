...
* Learn about [Using Channeld](../../02_usage/index.md).
...
3. Start `channeld` with the `--snapshot` option, and the options listed in the [`state_history_plugin`](index.md).
...
6. Do not stop `channeld` until it has received at least 1 block from the network, or it won't be able to restart.

## Remarks

If `channeld` fails to receive blocks from the network, then try the above using `net_api_plugin`. Use `chan net disconnect` and `chan net connect` to reconnect nodes which timed out.
