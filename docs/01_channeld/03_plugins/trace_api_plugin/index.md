...
```sh
# command-line
channeld ... --plugin eosio::trace_api_plugin [options]
```

## Configuration Options

These can be specified from both the `channeld` command-line or the `config.ini` file:
...
```sh
# command-line
channeld ... --plugin eosio::chain_plugin [options]  \
           --plugin eosio::http_plugin [options]
```

## Configuration Example

Here is a `channeld` configuration example for the `trace_api_plugin` when tracing some Antelope reference contracts:

```sh
channeld --data-dir data_dir --config-dir config_dir --trace-dir traces_dir
--plugin eosio::trace_api_plugin 
--trace-rpc-abi=ra=abis/ra.abi 
--trace-rpc-abi=ra.token=abis/ra.token.abi 
--trace-rpc-abi=ra.token=abis/core.vaulta.abi 
--trace-rpc-abi=ra.msig=abis/ra.msig.abi 
--trace-rpc-abi=ra.wrap=abis/ra.wrap.abi
```
