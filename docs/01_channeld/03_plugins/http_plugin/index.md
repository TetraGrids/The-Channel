...
The `http_plugin` is a core plugin supported by both `channeld` and `keyd`. The plugin is required to enable any RPC API functionality provided by a `channeld` or `keyd` instance.
...
```sh
# command-line
channeld ... --plugin eosio::http_plugin [options]
 (or)
keyd ... --plugin eosio::http_plugin [options]
```
