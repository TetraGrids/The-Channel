---
content_title: Channeld Configuration
---

The plugin-specific options can be configured using either CLI options or a configuration file, `config.ini`. Channeld-specific options can only be configured from the command line. All CLI options and `config.ini` options can be found by running `channeld --help` as shown above.
...
`~/.local/share/eosio/channeld/config`

A custom `config.ini` file can be set by passing the `channeld` option `--config path/to/config.ini`.

## Channeld Example

The example below shows a typical usage of `channeld` when starting a block producing node:
...
The above `channeld` command starts a producing node by:

* enabling block production (`-e`)
* identifying itself as block producer "ra" (`-p`)
...
* redirecting both `stdout` and `stderr` to the `channeld.log` file
