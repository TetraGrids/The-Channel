 ...
 * `signature_provider_ssm_plugin`), installed at `eosio/signature_provider_aws/aws_common.hpp`.
 *
 ...
 * cache that matters.
 *
 * @throws eosio::chain::plugin_config_exception when no region can be resolved
 ...
 *     timeouts, service-unavailable) -> `eosio::chain::signing_transient_exception`.
 ...
 *     parameters) -> `eosio::chain::plugin_config_exception`. Retrying will not
...
[[noreturn]] void throw_aws_error(std::string_view service, std::string_view op, std::string_view resource_noun,
   ...
   std::string message = "AWS " + std::string(service) + " " + std::string(op) + " for " + std::string(resource_noun) +
      " \"" + std::string(resource_id) + "\" failed: " + std::string(magic_enum::enum_name(err.GetErrorType())) +
      " (status " + std::to_string(magic_enum::enum_integer(err.GetResponseCode())) + ", " +
      err.GetExceptionName() + ") [" + (transient ? "transient, retryable" : "permanent") + "]: " + err.GetMessage();

   if (transient) {
      FC_THROW_EXCEPTION(chain::signing_transient_exception, "${m}", ("m", message));
   }
   if (!permanent_hint.empty()) {
      message += " " + std::string(permanent_hint);
   }
   FC_THROW_EXCEPTION(chain::plugin_config_exception, "${m}", ("m", message));
}
