...
struct controller_impl {
    ...
    wasmif( conf.wasm_runtime, conf.eosvmoc_tierup, db, main_thread_timer, conf.state_dir, conf.eosvmoc_config, !conf.profile_accounts.empty() )
   {
      ...
      set_activation_handler<builtin_protocol_feature_t::em_ed_keys>();
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   block_state_pair read_from_snapshot( const snapshot_reader_ptr& snapshot, uint32_t blog_start, uint32_t blog_end ) {
      ...
      if (header.version >= v8::minimum_version) {
         // loading a snapshot saved by The Channel 1.0.1 and above.
         // ---------------------------------------------------
         ...
         // snapshot created with The Channel 1.0.0, which was very soon superseded by The Channel 1.0.1
         // and a new snapshot format.
         // ----------------------------------------------------------------------------------
         EOS_THROW(snapshot_exception, "v7 snapshots are not supported anymore in The Channel 1.0.1 and above");
      } else {
...
void controller::preactivate_feature( const digest_type& feature_digest, bool is_trx_transient ) {
   ...
   // Even if they don't recognize the feature, the producer could change their channeld code to treat it like an
...
template<>
void controller_impl::on_activation<builtin_protocol_feature_t::em_ed_keys>() {
   db.modify( db.get<protocol_state_object>(), [&]( auto& ps ) {
