...
namespace fc::crypto::blslib {
   ...
   public:
      ...
      std::strong_ordering operator<=>(const bls_public_key& rhs) const {
         // memcmp: arrays are uint8_t, so byte order == lexicographical order
         const int c = std::memcmp(_affine_non_montgomery_le.data(), rhs._affine_non_montgomery_le.data(),
                                   _affine_non_montgomery_le.size());
