...
secure_output_file::secure_output_file(std::filesystem::path file_path)
   ...
   auto [fd_, temp_path_] = open_temp_file(file_path_);
   const int fd = fd_; // AppleClang 14 cannot capture structured bindings
   const std::filesystem::path temp_path = temp_path_;
   auto cleanup_on_failure = fc::make_scoped_exit([fd, &temp_path]() noexcept {
