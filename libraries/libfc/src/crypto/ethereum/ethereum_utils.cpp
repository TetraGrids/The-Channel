...
std::string trim_public_key(const std::string& hex) {
   ...
   FC_ASSERT(std::find(public_key_string_lengths.begin(), public_key_string_lengths.end(), len) != public_key_string_lengths.end(), "Invalid public key length(${len}): ${hex}", ("len", len)("hex", hex));
...
fc::em::public_key to_em_public_key(const std::string& pubkey_hex) {
   ...
   default:
      FC_ASSERT(false,
                "Invalid EM public key byte count (${count}). Expected 33 (compressed), 64 or 65 (uncompressed).",
                ("count", pubkey_byte_count));
   }
}
...
fc::em::private_key to_em_private_key(const std::string& privkey_hex) {
   ...
   FC_ASSERT(privkey_bytes.size() == fc::data_size(sk), "Invalid private key size, expected ${expected}, received ${received}",
             ("expected", fc::data_size(sk))("received", privkey_bytes.size()));
...
fc::em::compact_signature to_em_signature(const std::string& signature_hex) {
   ...
   FC_ASSERT(signature_bytes.size() == std::tuple_size_v<fc::em::compact_signature>,
             "Invalid signature size, expected ${expected}, received ${received} character hex string",
             ("expected", std::tuple_size_v<fc::em::compact_signature>)("received", signature_bytes.size()));
