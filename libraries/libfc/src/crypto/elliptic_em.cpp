...
void to_variant(const em::public_key& var, variant& vo) {
   auto ser = var.serialize();
   vo = fc::to_hex(ser.data(), ser.size());
}

void from_variant(const variant& var, em::public_key& vo) {
   auto bytes = fc::from_hex(var.as<std::string>());
   FC_ASSERT(bytes.size() == sizeof(em::public_key_data), "Invalid em public key hex size ${len}", ("len", bytes.size()));
   em::public_key_data dat{};
   std::memcpy(dat.data(), bytes.data(), bytes.size());
