...
def readlogs(node_num, net_latency, channeldLogPath):
    filename = channeldLogPath
...
specificExtraChanneldArgs[producerNodeId]=" --p2p-listen-endpoint 0.0.0.0:{}".format(9876+producerNodeId)
specificExtraChanneldArgs[syncingNodeId]="--p2p-peer-address 0.0.0.0:{}".format(9876+producerNodeId)

try:
    TestHelper.printSystemInfo("BEGIN")
    traceChanneldArgs=" --plugin eosio::producer_plugin --produce-block-offset-ms 0 --plugin eosio::net_plugin --net-threads 1"
    if cluster.launch(pnodes=1, totalNodes=totalNodes, totalProducers=1, specificExtraChanneldArgs=specificExtraChanneldArgs, extraChanneldArgs=traceChanneldArgs) is False:
    ...
    testSuccessful=readlogs(syncingNodeId, latency, cluster.channeldLogPath)
    if platform.system() == 'Darwin':
