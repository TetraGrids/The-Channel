...
try:
    ...
    if cluster.launch(pnodes=pnodes, totalNodes=total_nodes, activateIF=True, topo=topo, delay=delay,extraChanneldArgs=extraChanneldArgs ) is False:
    ...
    trx = {
        "actions": [{"account": "ra.token","name": "transfer",
    ...
    results = node.pushMessage('ra.token', 'transfer', '{"from": "account1","to": "account2","quantity": "1.0001 SYS","memo": "act1"}', '--dry-run -p account1@active')
    assert(results[0])
