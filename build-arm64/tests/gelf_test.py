...
###############################################################################################
# This test starts channeld process which is configured with GELF logging endpoint as localhost, 
...
start_channeld_cmd = shlex.split(f"{Utils.EosServerPath} -e -p eosio --data-dir={data_dir} --config-dir={config_dir}")
if os.path.exists(data_dir):
    shutil.rmtree(data_dir)
os.makedirs(data_dir)
if not os.path.exists(config_dir):
    os.makedirs(config_dir)
channeld = Node(TestHelper.LOCAL_HOST, TestHelper.DEFAULT_PORT, node_id, config_dir, data_dir, start_channeld_cmd, unstarted=True)
...
try:
  ...
  channeld.launchUnstarted(waitForAlive=False)
  time.sleep(channeld_run_time_in_sec)
finally:
   cleanup()
