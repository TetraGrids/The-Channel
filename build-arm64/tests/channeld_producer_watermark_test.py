...
###############################################################
# channeld_producer_watermark_test
# --dump-error-details <Upon error print etc/eosio/node_*/config.ini and channeld_producer_watermark_test<pid>/node_*/stderr.log to stdout>
# --keep-logs <Don't delete TestLogs/channeld_producer_watermark_test<pid>/node_* folders upon test completion>
###############################################################
...
def setProds(sharedProdKey):
    ...
    trans=cluster.biosNode.pushMessage("ra", "setprods", setProdsStr, opts)
...
try:
    ...
    while tries > 0:
        ...
        info = node.getInfo()
        if node.infoValid and node.lastRetrievedHeadBlockProducer != "ra":
            break
        time.sleep(1)
        tries = tries-1
    if tries == 0:
