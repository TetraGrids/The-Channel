...
#define MAKE_READ_WASM_ABI(CN,C, D)                                                         \
   static std::vector<uint8_t> CN ## _wasm() {                                              \
      std::string s;                                                                        \
      fc::read_file_contents("/Users/fresh/Dropbox/htdocs/The-Channel/build-verify/unittests/" #D "/" #C "/" #C ".wasm", s); \
      return std::vector<uint8_t>(s.begin(), s.end());                                      \
   }                                                                                        \
   static std::string CN ## _abi() {                                                        \
      std::string s;                                                                        \
      fc::read_file_contents("/Users/fresh/Dropbox/htdocs/The-Channel/build-verify/unittests/" #D "/" #C "/" #C ".abi", s);  \
      return s;                                                                             \
   }
...
namespace eosio {
   namespace testing {
      struct test_contracts {
         ...
         MAKE_READ_WASM_ABI(eosio_msig,   ra.msig, contracts)
         MAKE_READ_WASM_ABI(eosio_system, ra.system, contracts)
         MAKE_READ_WASM_ABI(eosio_token,  ra.token, contracts)
         MAKE_READ_WASM_ABI(eosio_wrap,   ra.wrap,   contracts)
         ...
         MAKE_READ_WASM_ABI(sync_caller,           sync_caller,           test-contracts)
         MAKE_READ_WASM_ABI(sync_callee,           sync_callee,           test-contracts)
         MAKE_READ_WASM_ABI(sync_callee1,          sync_callee1,          test-contracts)
