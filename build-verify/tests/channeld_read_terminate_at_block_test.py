...
###############################################################
# channeld_read_terminate_at_block_test
#
...
def executeSnapshotBlocklogTest(cluster, testNodeId, resultMsgs, nodeArgs, termAtBlock):
    ...
    errFileName=f"{cluster.channeldLogPath}/node_{str(testNodeId).zfill(2)}/stderr.txt"
...
try:
    ...
    specificChanneldArgs.update(regularChanneldArgs)
    specificChanneldArgs.update(replayChanneldArgs)
    ...
    for nodeId in replayChanneldArgs:
    ...
    for nodeId, nodeArgs in regularChanneldArgs.items():
        success = executeTest(
            cluster,
            nodeId,
            nodeArgs,
    ...
    if success:
        for nodeId, nodeArgs in replayChanneldArgs.items():
    ...
    if success:
        for nodeId, nodeArgs in {**regularChanneldArgs, **replayChanneldArgs}.items():
