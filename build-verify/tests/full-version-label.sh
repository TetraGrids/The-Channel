#!/bin/bash
set -eo pipefail
# The purpose of this test is to ensure that the output of the "channeld --full-version" command matches the version string defined by our CMake files
echo '##### Channeld Full Version Label Test #####'
# orient ourselves
...
# get channeld version
ACTUAL=$($BUILD_ROOT/bin/channeld --full-version)
EXIT_CODE=$?
# verify 0 exit code explicitly
if [[ $EXIT_CODE -ne 0 ]]; then
    echo "Channeld produced non-zero exit code \"$EXIT_CODE\"."
    exit $EXIT_CODE
fi
# test version
