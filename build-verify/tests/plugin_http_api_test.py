...
class HttpCategoryConfig:
    ...
    def channeldArgs(self):
        if not self.ports:
            return ""
        args = list(map(
            lambda item: f"--http-category-address {item[0]},{TestHelper.LOCAL_HOST}:{item[1]}", self.ports.items()))

        # at this moment, chan cannot handle split api endpoints, put all these categories in the same default endpoint
        chan_categories = ["chain_rw",
                            "producer_ro", "producer_rw", "trace_api"]
        args += list(map(
            lambda item: f"--http-category-address {item},{TestHelper.LOCAL_HOST}:{HttpCategoryConfig.default_port}", chan_categories))
...
class PluginHttpTest(unittest.TestCase):
    ...
    keyd = WalletMgr(True, TestHelper.DEFAULT_PORT, TestHelper.LOCAL_HOST, TestHelper.DEFAULT_WALLET_PORT, TestHelper.LOCAL_HOST)
    node_id = 1
    ...
    def endpoint(self, category: str):
        return f'http://{self.channeld.host}:{category_config.port(category)}'
   ...
   # kill channeld. keyd shuts down automatically
    def killNodes(self):
        self.channeld.kill(signal.SIGTERM)
    ...
    def startEnv(self) :
        ...
        channeld_plugins = "--plugin eosio::" +  " --plugin eosio::".join(plugin_names)
        channeld_flags = (" --data-dir=%s --config-dir=%s --trace-dir=%s --trace-no-abis --access-control-allow-origin=%s "
        ...
        channeld_flags += category_config.channeldArgs()

        start_channeld_cmd = ("%s -e -p eosio %s %s ") % (Utils.EosServerPath, channeld_plugins, channeld_flags)
        self.channeld = Node(TestHelper.LOCAL_HOST, TestHelper.DEFAULT_PORT, self.node_id, self.data_dir, self.config_dir, shlex.split(start_channeld_cmd), walletMgr=self.keyd)
        time.sleep(self.sleep_s*2)
        self.channeld.waitForBlock(1, timeout=30)

    def activateAllBuiltinProtocolFeatures(self):
        self.channeld.activatePreactivateFeature()
        ...
        self.keyd.create(testWalletName, walletAccounts)

        retMap = self.channeld.publishContract(eosioAccount, contractDir, wasmFile, abiFile, waitForTransBlock=True)

        self.channeld.activateAllBuiltinProtocolFeature()
    ...
    def test_ChainApi(self) :
        ...
        ret_json = self.channeld.processUrllibRequest(resource, command, endpoint=endpoint)
        ...
        ret_json = self.channeld.processUrllibRequest(resource, command, endpoint=self.endpoint("producer_ro"))
        self.assertIn("server_version", ret_json["payload"])
        # get_info with empty content parameter
        ret_json = self.channeld.processUrllibRequest(resource, command, self.empty_content_dict, endpoint=endpoint)
        self.assertIn("server_version", ret_json["payload"])
        # get_info with invalid parameter
        ret_json = self.channeld.processUrllibRequest(resource, command, self.http_post_invalid_param, endpoint=endpoint)
        self.assertEqual(ret_json["code"], 400)

        # activate the builtin protocol features and get some useful data
        self.activateAllBuiltinProtocolFeatures()
        allProtocolFeatures = self.channeld.getSupportedProtocolFeatures()
        ...
        ret_json = self.channeld.processUrllibRequest(resource, command, payload, endpoint=endpoint)
        self.assertEqual(type(ret_json["payload"]["activated_protocol_features"]), list)
        ...
        payload = {"limit":1} # ignored by channeld
        ret_json = self.channeld.processUrllibRequest(resource, command, payload, endpoint=endpoint)
        self.assertEqual(type(ret_json["payload"]["activated_protocol_features"]), list)
        self.assertEqual(len(ret_json["payload"]["activated_protocol_features"]), ACT_FEATURE_CURRENT_EXPECTED_TOTAL)
        for dict_feature in ret_json["payload"]["activated_protocol_features"]:
        ...
        payload = {"limit":ACT_FEATURE_CURRENT_EXPECTED_TOTAL} # ignored by channeld
        ret_json = self.channeld.processUrllibRequest(resource, command, payload, endpoint=endpoint)
        self.assertEqual(type(ret_json["payload"]["activated_protocol_features"]), list)
        self.assertEqual(len(ret_json["payload"]["activated_protocol_features"]), ACT_FEATURE_CURRENT_EXPECTED_TOTAL)
        for dict_feature in ret_json["payload"]["activated_protocol_features"]:
        ...
        payload = {"limit":999999} # ignored by channeld
        ret_json = self.channeld.processUrllibRequest(resource, command, payload, endpoint=endpoint)
        self.assertEqual(type(ret_json["payload"]["activated_protocol_features"]), list)
        self.assertEqual(len(ret_json["payload"]["activated_protocol_features"]), ACT_FEATURE_CURRENT_EXPECTED_TOTAL)
        for dict_feature in ret_json["payload"]["activated_protocol_features"]:
        ...
        self.assertEqual(ret_json["code"], 400)
        self.assertEqual(ret_json["error"]["code"], 3200006)
        self.channeld.waitForNextBlock()
        # get_block_header_state with reversible block
        head_block_num = self.channeld.getHeadBlockNum()
        payload = {"block_num_or_id":head_block_num}
        ret_json = self.channeld.processUrllibRequest(resource, command, payload, endpoint=endpoint)
        self.assertEqual(ret_json["payload"]["block_num"], head_block_num)
        self.assertEqual(ret_json["payload"]["header"]["producer"], "ra")
        # and by id
        ret_json = self.channeld.processUrllibRequest(resource, command, {"block_num_or_id":ret_json["payload"]["id"]}, endpoint=endpoint)
        self.assertEqual(ret_json["payload"]["block_num"], head_block_num)
        self.assertEqual(ret_json["payload"]["header"]["producer"], "ra")
        # get_block_header_state with irreversible block
        lib_block_num = self.channeld.getIrreversibleBlockNum()
        ...
        payload = {"code":"ra.token", "account":"unknown"}
        ret_json = self.channeld.processUrllibRequest(resource, command, payload, endpoint=endpoint)
        self.assertEqual(ret_json["code"], 400)
        ...
        payload = {"code":"ra.token","symbol":"SYS"}
        ret_json = self.channeld.processUrllibRequest(resource, command, payload, endpoint=endpoint)
        self.assertEqual(ret_json["code"], 400)
        ...
        payload_no_actions = {"expiration":"2020-08-01T07:15:49",
                ...
                "context_free_data": []}
        ret_json = self.channeld.processUrllibRequest(resource, command, payload_no_actions)
        self.assertEqual(ret_json["code"], 400)
        self.assertEqual(ret_json["error"]["code"], 3200006)
        ...
        payload_invalid_actions = {"expiration":"2020-08-01T07:15:49",
                ...
                "signatures": ["SIG_K1_KeqfqiZu1GwUxQb7jzK9Fdks6HFaVBQ9AJtCZZj56eG9qGgvVMVtx8EerBdnzrhFoX437sgwtojf2gfz6S516Ty7c22oEp"],
                "context_free_data": []}
        ret_json = self.channeld.processUrllibRequest(resource, command, payload_invalid_actions)
        self.assertEqual(ret_json["code"], 400)
        self.assertEqual(ret_json["error"]["code"], 3200006)
        ...
        payload = {"expiration":"2020-08-01T07:15:49",
                   ...
                   "delay_sec":0,
                   "context_free_actions":[],
                   "actions":[{"account":"ra.token","name": "transfer","authorization": [{"actor": "han","permission": "active"}],"data": "000000000000a6690000000000ea305501000000000000000453595300000000016d"}],
                   "transaction_extensions": [],
                   "signatures": ["SIG_K1_KeqfqiZu1GwUxQb7jzK9Fdks6HFaVBQ9AJtCZZj56eG9qGgvVMVtx8EerBdnzrhFoX437sgwtojf2gfz6S516Ty7c22oEp"],
                   "context_free_data": []}
        ret_str = self.channeld.processUrllibRequest(resource, command, payload, returnType=ReturnType.raw, endpoint=endpoint).decode('ascii')
        ...
        payload_hex = {"expiration":"2020-08-01T07:15:49",
                ...
                "delay_sec":0,
                "context_free_actions":[],
                "actions":[{"account":"ra.token","name": "transfer","authorization": [{"actor": "han","permission": "active"}],
                ...
                "signatures": ["SIG_K1_KeqfqiZu1GwUxQb7jzK9Fdks6HFaVBQ9AJtCZZj56eG9qGgvVMVtx8EerBdnzrhFoX437sgwtojf2gfz6S516Ty7c22oEp"],
                "context_free_data": []}
        ret_str = self.channeld.processUrllibRequest(resource, command, payload_hex, returnType=ReturnType.raw).decode('ascii')
        self.assertEqual(ret_str, "\"0be762a6406bab15530e87f21e02d1c58e77944ee55779a76f4112e3b65cac48\"")
    ...
    def test_NetApi(self) :
        ...
        command = "connections"
        ret_str = self.channeld.processUrllibRequest(resource, command, returnType=ReturnType.raw, endpoint=endpoint).decode('ascii')
        self.assertIn("\"peer\":\"localhost:9011\"", ret_str)
        # connections with empty content parameter
        ret_str = self.channeld.processUrllibRequest(resource, command, self.empty_content_dict, returnType=ReturnType.raw, endpoint=endpoint).decode('ascii')
        self.assertIn("\"peer\":\"localhost:9011\"", ret_str)
    ...
    def test_TraceApi(self) :
        ...
        ret_json = self.channeld.processUrllibRequest(resource, command)
        self.assertEqual(ret_json["code"], 400)
        # get_transaction_trace with empty content parameter
        ret_json = self.channeld.processUrllibRequest(resource, command, self.empty_content_dict)
        self.assertEqual(ret_json["code"], 400)
        # get_transaction_trace with invalid parameter
        ret_json = self.channeld.processUrllibRequest(resource, command, self.http_post_invalid_param)
        self.assertEqual(ret_json["code"], 400)
        ...
        ret_json = self.channeld.processUrllibRequest(resource, command, payload)
        self.assertEqual(ret_json["code"], 404)
        self.assertEqual(ret_json["error"]["code"], 0)
    ...
    def test_prometheusApi(self) :
        ...
        command = "metrics"
        endpoint = self.endpoint(resource)
        ret_text = self.channeld.processUrllibRequest(resource, command, returnType = ReturnType.raw, method="GET", endpoint=endpoint).decode()
        ...
        self.assertTrue(int(metrics["channeld_head_block_num"]) > 1)
        self.assertTrue(int(metrics["channeld_blocks_produced"]) > 1)
        self.assertTrue(int(metrics["channeld_last_irreversible"]) > 1)

        ret = self.channeld.processUrllibRequest(resource, "m", returnType = ReturnType.raw, method="GET", silentErrors= True, endpoint=endpoint)
    ...
    def test_multipleRequests(self):
        ...
        try:
            ...
            print(e)
            Utils.errorExit("Failed to connect to channeld.")
