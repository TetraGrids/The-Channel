...
class netUtil:
    def __init__(self):
        self.prometheusMetrics = {
            ('channeld_info', 'server_version'): 'Channeld Version ID:',
            ('channeld_info', 'chain_id'): 'Chain ID:',
            ('channeld_info', 'server_version_string'): 'Channeld Version:',
            ('channeld_info', 'server_full_version_string'): 'Channeld Full Version:',
            ('channeld_info', 'earliest_available_block_num'): 'Earliest Available Block:',
            'channeld_head_block_num': 'Head Block Num:',
            'channeld_last_irreversible': 'LIB:',
            'channeld_p2p_clients': 'Inbound P2P Connections:',
            'channeld_p2p_peers': 'Outbound P2P Connections:',
            'channeld_blocks_incoming_total': 'Total Incoming Blocks:',
            'channeld_trxs_incoming_total': 'Total Incoming Trxs:',
            'channeld_blocks_produced_total': 'Blocks Produced:',
            'channeld_trxs_produced_total': 'Trxs Produced:',
            'channeld_scheduled_trxs_total': 'Scheduled Trxs:',
            'channeld_unapplied_transactions_total': 'Unapplied Trxs:',
            'channeld_p2p_dropped_trxs_total': 'Dropped Trxs:',
            'channeld_p2p_failed_connections_total': 'Failed P2P Connections:',
            'channeld_http_requests_total': 'HTTP Requests:',
        }
        self.ignoredPrometheusMetrics = [
            'channeld_exposer_scrapes_total',
            'channeld_exposer_transferred_bytes_total',
            'channeld_subjective_bill_account_size_total',
            'channeld_net_usage_us_total',
            'channeld_cpu_usage_us_total',
        ]
        ...
        parser = argparse.ArgumentParser(description='Terminal UI for monitoring channeld P2P connections',
        ...
        try:
            ...
            for family in text_string_to_metric_families(response.text):
                ...
                for sample in family.samples:
                    ...
                    elif sample.name == 'channeld_p2p_addr':
                    ...
                    elif sample.name == 'channeld_p2p_bytes_sent':
                        stats = bandwidths.get(connID, bandwidthStats())
                        stats.bytesSent = int(sample.value)
                        bandwidths[connID] = stats
                    elif fieldName == 'channeld_p2p_block_sync_bytes_sent':
                        stats = bandwidths.get(connID, bandwidthStats())
                        stats.blockSyncBytesSent = int(sample.value)
                        bandwidths[connID] = stats
                    elif sample.name == 'channeld_p2p_bytes_received':
                        stats = bandwidths.get(connID, bandwidthStats())
                        stats.bytesReceived = int(sample.value)
                        bandwidths[connID] = stats
                    elif sample.name == 'channeld_p2p_connection_start_time':
                        stats = bandwidths.get(connID, bandwidthStats())
                        stats.connectionStarted = int(sample.value)
                        bandwidths[connID] = stats
                    elif sample.name == 'channeld_p2p_connection_number':
                        pass
                    elif sample.name.startswith('channeld_p2p_'):
                        fieldName = sample.name[len('channeld_p2p_'):]
                    ...
                    if sample.name == 'channeld_p2p_bytes_sent' or sample.name == 'channeld_p2p_bytes_received' or sample.name == 'channeld_p2p_block_sync_bytes_sent':
                        now = time.time_ns()
