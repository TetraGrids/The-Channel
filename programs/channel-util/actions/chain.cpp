...
int chain_actions::run_subcommand_sstate() {
   ...
   if(opt->sstate_state_dir.empty()) {
      ...
      auto default_data_dir = root / "ra" / "channeld" / "data" ;
   ...
   try {
      ...
      FC_ASSERT(header.id == chainbase::header_id, "\"" + state_dir.string() + "\" database format not compatible with this version of channel-util");
