...
void log_non_default_options(const std::vector<bpo::basic_option<char>>& options) {
   ...
   // legacy <PUB_BLS>=KEY:<PVT_BLS> finalizer-provider form
   ...
   for (const auto& op : options) {
      ...
      for (auto i = op.value.cbegin(), b = op.value.cbegin(), e = op.value.cend(); i != e; ++i) {
         ...
         if (op.string_key == "signature-provider"s)
            v += redact_signature_provider_spec(*i); // 5-field manager spec; masks KEY:<private>
         else if (op.string_key == "finalizer-provider"s)
            v += mask_private(*i);
         else if (mask)
            v += "***";
         else
...
int main(int argc, char** argv)
{
   ...
   ilog("channeld started");

   try {
      appbase::scoped_app app;
      auto on_exit = fc::make_scoped_exit([&]() {
         ilog("${name} version ${ver} ${fv}",
              ("name", channeld::config::node_executable_name)("ver", app->version_string())
      ...
      app->set_default_data_dir(root / "ra" / channeld::config::node_executable_name / "data" );
      app->set_default_config_dir(root / "ra" / channeld::config::node_executable_name / "config" );
      http_plugin::set_defaults({
         .default_unix_socket_path = "",
         .default_http_port = 8888,
         .server_header = channeld::config::node_executable_name + "/" + app->version_string()
      });
      ...
      ilog("${name} version ${ver} ${fv}",
            ("name", channeld::config::node_executable_name)("ver", app->version_string())
            ("fv", app->version_string() == app->full_version_string() ? "" : app->full_version_string()) );
      ilog("${name} using configuration file ${c}", ("name", channeld::config::node_executable_name)("c", app->full_config_file_path().string()));
      ilog("${name} data directory is ${d}", ("name", channeld::config::node_executable_name)("d", app->data_dir().string()));
      ::detail::log_non_default_options(app->get_parsed_options());
      app->startup();
   ...
   ilog("${name} successfully exiting", ("name", channeld::config::node_executable_name));
   return SUCCESS;
}
