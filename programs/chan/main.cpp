/**
  ...
  @section intro Introduction to chan

  `chan` is a command line tool that interfaces with the REST api exposed by @ref channeld. In order to use `chan` you will need to
  have a local copy of `channeld` running and configured to load the 'eosio::chain_api_plugin'.

   chan contains documentation for all of its commands. For a list of all commands known to chan, simply run it with no arguments:
```
$ ./chan
Command Line Interface to EOSIO Client
Usage: programs/chan/chan [OPTIONS] SUBCOMMAND

Options:
  -h,--help                   Print this help message and exit
  -u,--url TEXT=http://localhost:8888/
                              the http URL where channeld is running
  --wallet-url TEXT=http://localhost:8888/
                              the http URL where keyd is running
...
Usage: ./chan create account [OPTIONS] creator name OwnerKey ActiveKey
...
//copy pasta from keyd's main.cpp
...
constexpr name eosio_token_name = "ra.token"_n;
...
bool abi_valid(const abi_def& abi, F&& f) {
   ...
   if (!abi_ver)
      ...
      std::cerr << std::forward<F>(f)() << " (" << abi_ver->str() << ") is higher than supported by chan ("
...
void ensure_keyd_running(CLI::App* app) {
    if (no_auto_keyd)
        return;
    // get, version, net, convert do not require keyd
    ...
    if (!std::filesystem::exists(binPath)) {
        binPath = parent_path.parent_path() / "keyd"/ key_store_executable_name;
    }
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->callback([this] {
            auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so chan can still work with old buggy channeld versions
                               // Change to voter.value when chan no longer needs to support channeld versions older than 1.5.0
                               ("limit", 1)
            );
            auto res = result.as<eosio::chain_apis::read_only::get_table_rows_result>();
            // Condition in if statement below can simply be res.rows.empty() when chan no longer needs to support channeld versions older than 1.5.0
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so chan can still work with old buggy channeld versions
                               // Change to newname.value when chan no longer needs to support channeld versions older than 1.5.0
                               ("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
...
struct activate_subcommand {
   ...
   std::string account_str = "ra";
   std::string permission_str = "ra";
   ...
   activate_subcommand(CLI::App* actionRoot) {
      ...
      activate->add_option("feature",  feature_name_str, localized("The name, can be found from \"chan get supported_protoctol_features\" command"))->required();
   ...
   if (!json_format) {
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of channeld responding to the request is different than core symbol built into chan
...
int main( int argc, char** argv ) {
   ...
   CLI::App app{"Command Line Interface to The Channel Client"};
   ...
   app.add_flag( "--no-auto-" + string(key_store_executable_name), no_auto_keyd, localized("Don't automatically launch a ${k} if one is not currently running", ("k", key_store_executable_name)));
   app.parse_complete_callback([&app]{ ensure_keyd_running(&app);});
   ...
   // create key
   auto create_key = create->add_subcommand("key", localized("Create a new keypair and print the public and private keys"))->callback( [&r1, &em, &ed, &key_file, &print_console](){
      if (key_file.empty() && !print_console) {
         std::cerr << "ERROR: Either indicate a file using \"--file\" or pass \"--to-console\"" << std::endl;
         return;
      }
      if ((int)r1 + (int)em + (int)ed > 1) {
         std::cerr << "ERROR: Only one of \"--r1\", \"--em\" or \"--ed\" may be passed" << std::endl;
         return;
      }

      auto key_type = r1 ? private_key_type::key_type::r1
                       : em ? private_key_type::key_type::em
                            : ed ? private_key_type::key_type::ed
                                 : private_key_type::key_type::k1;
      auto pk    = private_key_type::generate(key_type);
   ...
   create_key->add_flag( "--em", em, "Generate an Ethereum-style key (secp256k1/keccak, EM)"  );
   create_key->add_flag( "--ed", ed, "Generate an Ed25519 key (Solana-style, ED)"  );
   ...
   getCode->callback([&] {
      ...
      try {
         ...
         //see if this is an old channeld that doesn't support get_raw_code_and_abi
...
//                                ->check(CLI::ExistingFile);
   ...
   auto stopKeyd = wallet->add_subcommand("stop", localized("Stop ${k}.", ("k", key_store_executable_name)));
   stopKeyd->callback([] {
      const auto& v = call(wallet_url, keyd_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success keyd responds with empty object
   ...
   propose_action->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "ra.msig"_n, "propose"_n, variant_to_bin( "ra.msig"_n, "propose"_n, args ) }}, signing_keys_opt.get_keys());
   });
   ...
   review->callback([&] {
      const auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)
                                 ...
                                 // Less than ideal upper_bound usage preserved so chan can still work with old buggy channeld versions
                                 // Change to name(proposal_name).value when chan no longer needs to support channeld versions older than 1.5.0
                                 ("limit", 1)
                           );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;

      const auto& rows1 = result1.get_object()["rows"].get_array();
      // Condition in if statement below can simply be rows.empty() when chan no longer needs to support channeld versions older than 1.5.0
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, "ra.msig"_n, name(action), variant_to_bin( "ra.msig"_n, name(action), args ) }}, signing_keys_opt.get_keys());
   };
   ...
   invalidate->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "ra.msig"_n, "invalidate"_n, variant_to_bin( "ra.msig"_n, "invalidate"_n, args ) }}, signing_keys_opt.get_keys());
   });
   ...
   cancel->callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, "ra.msig"_n, "cancel"_n, variant_to_bin( "ra.msig"_n, "cancel"_n, args ) }}, signing_keys_opt.get_keys());
      }
   );
   ...
   exec->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "ra.msig"_n, "exec"_n, variant_to_bin( "ra.msig"_n, "exec"_n, args ) }}, signing_keys_opt.get_keys());
      }
   );
