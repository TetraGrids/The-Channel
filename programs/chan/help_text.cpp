...
You can check the contract's abi by using 'chan get code' command.)=====";
...
If you are currently using 'chan push action' command, try to add the relevant authority using -p option.)=====";
...
const char* error_advice_missing_wallet_api_plugin_exception =  "Ensure that you have keyd enabled on the \033[2m--wallet-url\033[0m\033[32m specified endpoint.";
      ...
      for (auto &log : e.get_log()) {
         ...
         if (!log.get_context().get_method().empty() && verbose_errors) {
            stack_trace += "\n" +
                           ...
                           std::to_string(log.get_context().get_line_number())  + " " +
