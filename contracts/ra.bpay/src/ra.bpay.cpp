...
void bpay::claimrewards( const name owner ) {
    ...
    eosio::token::transfer_action transfer( "ra.token"_n, { get_self(), "active"_n });
...
void bpay::on_transfer( const name from, const name to, const asset quantity, const string memo ) {
    ...
    // ignore eosio system incoming transfers (caused by bpay income transfers eosio => ra.bpay => producer)
    if ( from == "ra"_n) return;

    symbol system_symbol = rasystem::system_contract::get_core_symbol();

    check( quantity.symbol == system_symbol, "only core token allowed" );

    rewards_table _rewards( get_self(), get_self().value );
    rasystem::producers_table _producers( "ra"_n, "ra"_n.value );

    rasystem::global_state_singleton _global("ra"_n, "ra"_n.value);
