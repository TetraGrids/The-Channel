#include <eosio/signature_provider_kms_plugin/kms_signature_provider.hpp>

#include <eosio/signature_provider_aws/aws_common.hpp>
...
/// `eosio::sigprov::aws` (aws_common.hpp).
...
struct der_reader {
   ...
   [[nodiscard]] element next() {
      EOS_ASSERT(pos + 2 <= buf.size(), chain::plugin_config_exception,
      ...
      if ((len & der_length_long_form_bit) != 0) {
         ...
         EOS_ASSERT(len_octets >= 1 && len_octets <= sizeof(std::size_t),
                    chain::plugin_config_exception,
                    "Malformed KMS public-key DER: unsupported {}-octet length field",
                    len_octets);
         EOS_ASSERT(pos + len_octets <= buf.size(), chain::plugin_config_exception,
      ...
      EOS_ASSERT(len <= buf.size() - pos, chain::plugin_config_exception,
...
parse_spki_ec_point(std::span<const unsigned char> spki_der) {
   ...
   EOS_ASSERT(spki.tag == der_tag_sequence, chain::plugin_config_exception,
              "KMS public-key DER: expected outer SEQUENCE, got tag {:#x}",
              static_cast<unsigned>(spki.tag));
   EOS_ASSERT(top.at_end(), chain::plugin_config_exception,
   ...
   EOS_ASSERT(algo_oid.tag == der_tag_oid && curve_oid.tag == der_tag_oid,
              chain::plugin_config_exception,
              "KMS public-key DER: AlgorithmIdentifier is not a pair of OBJECT IDENTIFIERs");
   EOS_ASSERT(std::equal(algo_oid.content.begin(), algo_oid.content.end(), oid_ec_public_key.begin(), oid_ec_public_key.end()),
              chain::plugin_config_exception,
              "KMS public-key DER: algorithm is not id-ecPublicKey -- the KMS key is not an "
              "elliptic-curve key");
   EOS_ASSERT(std::equal(curve_oid.content.begin(), curve_oid.content.end(), oid_secp256k1.begin(), oid_secp256k1.end()),
              chain::plugin_config_exception,
   ...
   EOS_ASSERT(!subject_pk.content.empty() && subject_pk.content.front() == 0x00,
              chain::plugin_config_exception,
   ...
   EOS_ASSERT(point.size() == ec_point_uncompressed_len, chain::plugin_config_exception,
              "KMS public-key DER: EC point is {} bytes, expected {} (uncompressed 0x04 form)",
              point.size(), ec_point_uncompressed_len);
   EOS_ASSERT(point.front() == ec_point_uncompressed_prefix, chain::plugin_config_exception,
...
void verify_kms_pubkey(kms_signer_state& state) {
   ...
   EOS_ASSERT(kms_pubkey == state.expected_em_pubkey, chain::plugin_config_exception,
...
[[noreturn]] void throw_kms_error(std::string_view op, std::string_view key_id,
   ...
   // live in the shared `eosio::sigprov::aws::throw_aws_error`; see its doc
...
kms_key_ref parse_kms_spec(std::string_view spec_data) {
   EOS_ASSERT(!spec_data.empty(), chain::plugin_config_exception,
   ...
   if (spec_data.starts_with(kms_arn_prefix)) {
      ...
      EOS_ASSERT(parts.size() == sigprov::aws::arn_segment_count, chain::plugin_config_exception,
      ...
      EOS_ASSERT(!region.empty(), chain::plugin_config_exception,
                 "KMS ARN \"{}\" has empty region segment", spec_data);
      EOS_ASSERT(!account.empty(), chain::plugin_config_exception,
                 "KMS ARN \"{}\" has empty account-id segment", spec_data);
      EOS_ASSERT(tail.starts_with(tail_prefix_key) || tail.starts_with(tail_prefix_alias),
                 chain::plugin_config_exception,
      ...
      EOS_ASSERT(!name.empty(), chain::plugin_config_exception,
   ...
   if (colon == std::string_view::npos) {
      ...
      EOS_ASSERT(!sigprov::aws::looks_like_aws_region(spec_data), chain::plugin_config_exception,
   ...
   EOS_ASSERT(colon > 0, chain::plugin_config_exception,
              "KMS spec \"{}\" has empty region; drop the leading ':' for a region-less spec", spec_data);
   EOS_ASSERT(colon + 1 < spec_data.size(), chain::plugin_config_exception,
   ...
   EOS_ASSERT(sigprov::aws::looks_like_aws_region(region), chain::plugin_config_exception,
...
std::array<unsigned char, 64> der_to_compact(std::span<const unsigned char> der) {
   ...
   EOS_ASSERT(secp256k1_ecdsa_signature_parse_der(kms_secp_ctx(), &parsed, der.data(), der.size()) == 1,
              chain::plugin_config_exception,
   ...
   EOS_ASSERT(serialised == 1, chain::plugin_config_exception,
...
bool normalise_low_s(std::array<unsigned char, 64>& compact) {
   secp256k1_ecdsa_signature parsed{};
   EOS_ASSERT(secp256k1_ecdsa_signature_parse_compact(kms_secp_ctx(), &parsed, compact.data()) == 1,
              chain::plugin_config_exception,
...
unsigned char recover_v(const std::array<unsigned char, 64>& compact,
   ...
   for (unsigned char rec_id = 0; rec_id < 2; ++rec_id) {
      ...
      std::copy(compact.begin(), compact.end(), trial.begin());
...
fc::em::public_key spki_der_to_public_key(std::span<const unsigned char> spki_der) {
   ...
   std::copy(point.begin(), point.end(), uncompressed.begin());
...
kms_signer make_kms_signature_provider(const kms_key_ref&             ref,
   ...
   // for v1.
   EOS_ASSERT(key_type == fc::crypto::chain_key_type_ethereum,
   ...
   EOS_ASSERT(expected_pubkey.contains<fc::em::public_key_shim>(),
              chain::plugin_config_exception,
