 ...
 * `eosio/signature_provider_kms_plugin/kms_signature_provider.hpp`.
 *
 * This implements the `KMS:<key-ref>` spec grammar -- a third form alongside the built-in `KEY:` and `KEYD:` -- where
 * the signing key never leaves AWS. The manager plugin never links this code, so it carries no compile-time dependency
 * on the AWS SDK; `KMS:` support is enabled per binary+config with `plugin = eosio::signature_provider_kms_plugin`
 * (channeld links the plugin; enablement is the operator's choice). A host application that does not use the plugin can
 * instead link this code and call
 *   plugin.register_spec_handler("KMS", &eosio::sigprov::kms::create_kms_provider);
 ...
 * @throws eosio::chain::plugin_config_exception if the form is empty or
 *         malformed
 ...
 * @throws eosio::chain::plugin_config_exception on malformed DER
 ...
 * @throws eosio::chain::plugin_config_exception if neither parity recovers `expected`
 ...
 * @throws eosio::chain::plugin_config_exception if the DER is malformed, the
 ...
 *     service-unavailable) -> `eosio::chain::signing_transient_exception`. The
 ...
 *     bad parameters) -> `eosio::chain::plugin_config_exception`. Retrying will
 ...
 * @throws eosio::chain::signing_transient_exception if `err` is retryable
 * @throws eosio::chain::plugin_config_exception otherwise
 */
...
struct kms_signer {
   ...
   /// `eosio::chain::signing_transient_exception` (safe to retry
   /// with backoff); a permanent one throws
   /// `eosio::chain::plugin_config_exception` (fatal -- fix the configuration).
   ...
   /// `eosio::chain::plugin_config_exception`; a transient failure (throttle,
   /// timeout, `KMSInternal`) throws `eosio::chain::signing_transient_exception`
 ...
 * @throws eosio::chain::plugin_config_exception on malformed spec or
 *         key-type / pubkey mismatch
 * @throws eosio::chain::pending_impl_exception if the chain key type is not
