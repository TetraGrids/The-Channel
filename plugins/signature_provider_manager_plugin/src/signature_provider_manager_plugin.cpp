...
#include <eosio/signature_provider_manager_plugin/signature_provider_manager_plugin.hpp>
#include <eosio/http_client_plugin/http_client_plugin.hpp>

namespace eosio {
namespace {
constexpr auto option_name_keyd_timeout = "signature-provider-keyd-timeout";
...
constexpr std::string_view scheme_keyd = "KEYD";
...
void register_scheme_handler(std::string_view scheme, spec_handler handler, std::string_view plugin_name) {
   ...
   FC_ASSERT(scheme != scheme_key && scheme != scheme_keyd,
             "Cannot register built-in signature-provider scheme \"${scheme}\"", ("scheme", std::string(scheme)));
   FC_ASSERT(static_cast<bool>(handler), "signature-provider handler for \"${scheme}\" must not be empty",
             ("scheme", std::string(scheme)));
   FC_ASSERT(!plugin_name.empty(), "signature-provider scheme \"${scheme}\" must name its providing plugin",
             ("scheme", std::string(scheme)));
   ...
   if (auto it = registry.find(scheme); it != registry.end()) {
      FC_ASSERT(it->second.plugin_name == plugin_name,
                "signature-provider scheme \"${scheme}\" already registered by plugin \"${plugin}\"; \"${other}\" cannot also provide it",
                ("scheme", std::string(scheme))("plugin", it->second.plugin_name)("other", std::string(plugin_name)));
   }
...
public:
   /**
    * `keyd` request timeout
    */
   fc::microseconds _keyd_provider_timeout_us;
    ...
    * enabled.
    *
    * This is the enablement path channeld uses: provider plugins are `register_plugin<>`'d (available but off by default)
   ...
   fc::crypto::sign_fn make_keyd_signature_provider(const std::string& url_str, const chain::public_key_type& pubkey) const {
      fc::url keyd_url;
      if (boost::algorithm::starts_with(url_str, "unix://"))
         ...
         keyd_url = fc::url("unix", url_str.substr(7), fc::ostring(), fc::ostring(), fc::ostring(), fc::ostring(),
                            fc::ovariant_object(), std::optional<uint16_t>());
      else
         keyd_url = fc::url(url_str);

      return [to = _keyd_provider_timeout_us, keyd_url, pubkey](const chain::digest_type& digest) {
         fc::variant params;
         ...
         return app()
                ...
                .get_client()
                .post_sync(keyd_url, params, deadline)
                .as<chain::signature_type>();
      };
   }

   eosio::provider_spec_result create_provider_from_spec(
      ...
      FC_ASSERT(spec_parts.size() == max_split,
                "Provider spec '${spec}' is malformed. Format: '<spec type>:<spec data>'", ("spec", spec));

      auto spec_type_str = spec_parts[0];
      auto spec_data = spec_parts[1];
      FC_ASSERT(!spec_data.empty(),
                "Provider spec '${spec}' is malformed. Format: '<spec type>:<spec data>' has empty <spec data>",
                ("spec", spec));
      ...
      if (spec_type_str == scheme_key) {
         ...
         case chain_key_type_solana: {
            ...
            // extension handlers that also construct local-key providers (e.g. the ssm sub-library) share it.
            privkey = from_native_string_to_private_key(key_type, spec_data);
            break;
         }
         case chain_key_type_channel_bls: {
            // BLS finalizer keys are handled by producer_plugin's `finalizer-provider` option, not this plugin.
            FC_THROW_EXCEPTION(chain::plugin_config_exception,
                               "BLS keys are configured via producer_plugin's finalizer-provider option, not signature-provider");
         }
         case chain_key_type_sui: {
            ...
            FC_THROW_EXCEPTION(chain::plugin_config_exception, "Key type needs to be implemented: ${t}",
                               ("t", chain_key_type_reflector::to_fc_string(key_type)));
         }
         default: {
            FC_THROW_EXCEPTION(chain::config_parse_error, "Unknown or Unsupported chain kind: ${t}",
                               ("t", chain_key_type_reflector::to_fc_string(key_type)));
         }
         }

         FC_ASSERT(public_key == privkey.get_public_key(), "Private key does not match given public key for ${pub}",
                   ("pub", fc::json::to_log_string(public_key)));
         return {.signer = fc::crypto::make_local_sign_fn(privkey), .private_key = privkey};
      }

      if (spec_type_str == scheme_keyd) {
         return {.signer = make_keyd_signature_provider(spec_data, public_key)};
      }
      ...
      if (const auto* entry = sigprov::find_scheme_handler(spec_type_str)) {
         EOS_ASSERT(_enabled_plugins.contains(entry->plugin_name), chain::plugin_config_exception,
                    "Signature-provider scheme \"${scheme}\" is provided by plugin \"${plugin}\". Enable it with a "
                    "`plugin = ${plugin}` line in the config file (or pass `--plugin ${plugin}`).",
                    ("scheme", spec_type_str)("plugin", entry->plugin_name));
         return entry->handler(key_type, public_key, spec_data);
      }

      EOS_THROW(chain::plugin_config_exception,
                "Unknown provider type \"${scheme}\". Built-in types are KEY and KEYD; additional types "
                "(e.g. KMS, SSM) are provided by optional signature-provider plugins (enable with "
                "`plugin = ...`) -- no plugin in this binary provides \"${scheme}\".",
                ("scheme", spec_type_str));
   }
   ...
   fc::crypto::signature_provider_ptr set_provider(const fc::crypto::signature_provider_ptr& provider) {
      EOS_ASSERT(!_signing_providers_by_pubkey.contains(provider->public_key) &&
                 !_signing_providers_by_name.contains(provider->key_name),
                 chain::plugin_config_exception,
                 "A signature provider with key_name \"${name}\" or public_key \"${pub}\" already exists",
                 ("name", provider->key_name)("pub", fc::json::to_log_string(provider->public_key)));
   ...
   fc::crypto::signature_provider_ptr get_provider(const fc::crypto::signature_provider_id_t& key) {
      if (holds_alternative<std::string>(key)) {
         ...
         EOS_ASSERT(_signing_providers_by_name.contains(keyName), chain::plugin_config_exception,
                    "No signature provider exists with name \"${name}\"", ("name", keyName));

         return _signing_providers_by_name.at(keyName);
      }

      auto& pub_key = std::get<chain::public_key_type>(key);
      EOS_ASSERT(_signing_providers_by_pubkey.contains(pub_key), chain::plugin_config_exception,
                 "No signature provider exists with public key \"${pub}\"",
                 ("pub", fc::json::to_log_string(pub_key)));
   ...
   std::vector<fc::crypto::signature_provider_ptr> query_providers(
      ...
      std::vector<fc::crypto::signature_provider_ptr> out;
      for (const auto& [pub, entry] : _signing_providers_by_pubkey) {
         ...
         if (id_opt.has_value()) {
            if (std::holds_alternative<std::string>(id_opt.value())) {
               if (entry->key_name.find(std::get<std::string>(id_opt.value())) == std::string::npos)
                  continue;
            } else {
               if (!(entry->public_key == std::get<chain::public_key_type>(id_opt.value())))
                  continue;
            }
         }
         out.push_back(entry);
   ...
   void load_default_signature_provider_specs() {
      ...
      for (const auto& item : vo) {
         ...
         FC_ASSERT(inserted, "corrupt ${file}: duplicate default for key type \"${t}\"",
                   ("file", def_sig_prov_file.string())
                   ("t", fc::crypto::chain_key_type_reflector::to_string(key_type)));
      }
   ...
   void register_default_signature_providers(const std::vector<fc::crypto::chain_key_type_t>& key_types) {
      static constexpr std::array supported_key_types = {fc::crypto::chain_key_type_channel};
      ...
      for (const auto& key_type : key_types) {
         FC_ASSERT(std::find(supported_key_types.begin(), supported_key_types.end(), key_type) != supported_key_types.end(),
                   "Unsupported key type: ${t}", ("t", static_cast<uint8_t>(key_type)));
         ...
         auto key_name = std::string(fc::crypto::chain_key_type_reflector::to_string(key_type)) + "-default";
         FC_ASSERT(key_type == fc::crypto::chain_key_type_channel, "Unsupported key type: ${t}",
                   ("t", static_cast<uint8_t>(key_type)));
         fc::crypto::private_key privkey = fc::crypto::private_key::generate(fc::crypto::private_key::key_type::k1);
         auto pub_key_str = privkey.get_public_key().to_string({});
         auto spec = fc::crypto::to_signature_provider_spec(key_name,
                                                            fc::crypto::chain_kind_channel,
                                                            key_type,
                                                            pub_key_str,
         ...
         dlog("Registering default signature provider spec (type=${t})",
              ("t", fc::crypto::chain_key_type_reflector::to_string(key_type)));
         create_provider(spec);
      }
   ...
   parsed_provider_spec parse_provider_spec(const std::string& spec) const {
      using namespace fc::crypto;
      ...
      EOS_ASSERT(num_parts == 5 || num_parts == 4, chain::plugin_config_exception, "Invalid key spec: ${spec}",
                 ("spec", redact_signature_provider_spec(spec)));
      std::string key_name;
   ...
   fc::crypto::signature_provider_ptr create_provider(parsed_provider_spec parsed) {
      if (parsed.key_name.empty()) {
         parsed.key_name = "key-" + std::to_string(next_anon_key_counter());
      }
   ...
   fc::crypto::signature_provider_ptr create_configured_provider(const std::string& spec) {
   ...
   fc::crypto::signature_provider_ptr create_provider(const std::string& key_name,
      ...
      case chain_key_type_sui: {
         FC_THROW_EXCEPTION(chain::plugin_config_exception, "Key type: ${t}",
                            ("t", chain_key_type_reflector::to_fc_string(key_type)));
      }
      default: {
         FC_THROW_EXCEPTION(chain::config_parse_error, "Unknown or Unsupported chain kind: ${t}",
                            ("t", static_cast<uint8_t>(target_chain)));
      }
      }
    ...
    *
    * Built-in `KEY` and `KEYD` are not registrable. Each non-built-in scheme may be registered at most once. This is
    ...
    */
   void register_spec_handler(std::string scheme, eosio::spec_handler handler) {
      FC_ASSERT(!scheme.empty(), "spec handler scheme must not be empty");
      FC_ASSERT(scheme != scheme_key && scheme != scheme_keyd,
                "Cannot override built-in spec handler \"${scheme}\"", ("scheme", scheme));
      FC_ASSERT(static_cast<bool>(handler),
                "spec handler for \"${scheme}\" must not be empty", ("scheme", scheme));
      auto [it, inserted] = _spec_handlers.try_emplace(std::move(scheme), std::move(handler));
      FC_ASSERT(inserted, "spec handler \"${scheme}\" already registered", ("scheme", it->first));
   }

   /**
    * Run the opt-in startup-probe pass.
    *
    * For every provider whose handler attached a `startup_probe`, invoke that probe. Today only the KMS handler does
    * so -- the probe issues a single `GetPublicKey` call that resolves AWS credentials, warms the client, and verifies
    * the KMS key matches the pinned public key.
    *
   ...
   void run_startup_probes() {
      ...
      ilog("Running signature-provider startup probes for ${n} signing key(s)", ("n", probes.size()));
      ...
      for (auto& probe : probes) {
         try {
            ...
            wlog("Signature-provider startup probe: transient error for one "
                 "key, deferring its check to the first sign: ${e}",
      ...
      if (deferred == 0) {
         ...
         ilog("Signature-provider startup probes passed; ${n} key(s) hit a "
...
private:
    ...
    * (producer lookups) is read-only, and those threads are spawned by dependent plugins'
    ...
    */
   std::map<std::string, eosio::spec_handler> _spec_handlers{};
};
...
void signature_provider_manager_plugin::assert_pre_startup(std::string_view operation) const {
   ...
   EOS_ASSERT(current_state == registered || current_state == initialized, chain::plugin_config_exception,
              "signature_provider_manager_plugin::${op} called after startup: the provider set is immutable "
...
void signature_provider_manager_plugin::set_program_options(options_description&, options_description& cfg) {
   cfg.add_options()(
      option_name_keyd_timeout, boost::program_options::value<int32_t>()->default_value(5),
      "Limits the maximum time (in milliseconds) that is allowed for sending requests to a keyd provider for signing");
   cfg.add_options()(
      ...
      "`<name>,<chain-kind>,<key-type>,<public-key>,<private-key-provider-spec>`. "
      "Provider types KEY:<private-key> and KEYD:<url> are built in; additional "
      "schemes are provided by optional signature-provider plugins, enabled with a "
      "`plugin =` line: `plugin = eosio::signature_provider_ssm_plugin` for "
      "SSM:[<region>:]<parameter-name> (key fetched once from AWS SSM Parameter Store "
      "at startup); `plugin = eosio::signature_provider_kms_plugin` for KMS:<key-ref> "
...
const char* signature_provider_manager_plugin::signature_provider_help_text() const {
   return "Key=Value pairs in the form <public-key>=<provider-spec>\n"
      ...
      "   <chain-kind>           chain where the key will be used (channel, ethereum, solana)\n\n"
      "   <key-type>             key format to parse (channel, ethereum, solana)\n\n"
      "   <public-key>           is a string form of a valid <key-type>\n\n"
      "   <provider-spec>        is a string in the form <provider-type>:<data>\n\n"
      "       <provider-type>    KEY and KEYD are built in. Additional types are provided\n"
      ...
      "       KEYD:<url>         is the URL where keyd is available and the appropriate wallet(s) are unlocked\n\n"
      ...
      "                          `plugin = eosio::signature_provider_ssm_plugin`.\n\n"
      ...
      "                          `plugin = eosio::signature_provider_kms_plugin`.\n\n"
...
void signature_provider_manager_plugin::plugin_initialize(const variables_map& options) {
   if (options.contains(option_name_keyd_timeout))
      my->_keyd_provider_timeout_us = fc::milliseconds(options.at(option_name_keyd_timeout).as<int32_t>());
   ...
   // this plugin), so the full provider set exists by the time chain_plugin / producer_plugin look --
   ...
   if (options.contains(option_name_provider)) {
      ...
      for (const auto& spec : specs) {
         dlog("Registering signature provider from spec: ${spec}", ("spec", redact_signature_provider_spec(spec)));
         auto provider = my->create_configured_provider(spec);
         dlog("Registered signature provider (${name}): ${pub}",
              ("name", provider->key_name)("pub", provider->public_key.to_string({})));
      }
   }
}
...
void signature_provider_manager_plugin::register_spec_handler(
   std::string scheme, eosio::spec_handler handler) {
...
fc::crypto::signature_provider_ptr get_signature_provider(const fc::crypto::signature_provider_id_t& id,
   ...
   FC_ASSERT(providers.size() == 1, "Expected exactly one provider for query \"${q}\", found ${n}",
             ("q", fc::json::to_log_string(id))("n", providers.size()));
   return providers[0];
}
