 ...
 *     (e.g. `KEY:`); empty for remote signers (`KEYD:`, `KMS:`, ...).
 ...
 * `KEYD:` schemes are not registrable -- they are handled directly by the plugin.
 */
 ...
 * is the provider. Only a `KEY:<private-key>` provider embeds secret key material; `KEYD:`/other providers reference
...
public:
    ...
    *
    *   - `KEY:` / `KEYD:` are built in and handled directly.
    ...
    * full provider set exists by the time chain_plugin / producer_plugin look, independent of `--plugin` ordering.
    */
    ...
    *
    * @param spec A string specification in the format "<name>,<chain-kind>,<key-type>,<public-key>,<provider-spec>"
    ...
    * @param target_chain The target blockchain protocol (e.g., channel, ethereum)
    * @param key_type The type of key (e.g., channel, ethereum, solana)
   ...
   /**
    * Check that a provider exists for every criterion in `range` (each element a
    * `signature_provider_id_t`, `chain_kind_t`, or `chain_key_type_t`).
    */
   template <typename R>
   bool has_signature_providers(const R& range) {
      for (const auto& criteria : range) {
         if constexpr (std::is_same_v<std::decay_t<decltype(criteria)>, fc::crypto::signature_provider_id_t>) {
            if (query_providers(criteria).empty()) return false;
         } else if constexpr (std::is_same_v<std::decay_t<decltype(criteria)>, fc::crypto::chain_kind_t>) {
            if (query_providers(std::nullopt, criteria, std::nullopt).empty()) return false;
         } else if constexpr (std::is_same_v<std::decay_t<decltype(criteria)>, fc::crypto::chain_key_type_t>) {
            if (query_providers(std::nullopt, std::nullopt, criteria).empty()) return false;
         } else {
    ...
    *
    * Built-in schemes (`KEY:`, `KEYD:`) are not registrable -- attempting to register them throws. Each non-built-in
 ...
 * the manager is `APPBASE_PLUGIN_REQUIRES`-d by every consumer (chain_plugin, producer_plugin), so its
