#include <eosio/signature_provider_ssm_plugin/ssm_signature_provider.hpp>
...
/// same boundary as the kms-plugin sibling). The service-agnostic ARN pieces
/// (`arn:` lead-in, segment count / indices, the case-insensitive prefix
/// test) come from `eosio::sigprov::aws` (aws_common.hpp).
...
ssm_param_ref parse_ssm_spec(std::string_view spec_data) {
   ...
   if (spec_data.starts_with(ssm_arn_prefix)) {
      ...
      EOS_ASSERT(tail.starts_with(tail_prefix_parameter), chain::plugin_config_exception,
                 "SSM parameter ARN tail must start with 'parameter/', got \"{}\" in \"{}\"",
                 tail, spec_data);
      EOS_ASSERT(tail.size() > tail_prefix_parameter.size(), chain::plugin_config_exception,
      ...
      // (same account-identity reasoning as the kms plugin's key ARNs).
   ...
   EOS_ASSERT(spec_data.front() != ':', chain::plugin_config_exception,
...
sysio::provider_spec_result create_ssm_provider_with_fetcher(fc::crypto::chain_key_type_t  key_type,
   ...
   EOS_ASSERT(fetched.type == parameter_type::SecureString, chain::plugin_config_exception,
   ...
   EOS_ASSERT(!value.empty(), chain::plugin_config_exception,
   ...
   // no secret.
   EOS_ASSERT(expected_pub == privkey.get_public_key(), chain::plugin_config_exception,
