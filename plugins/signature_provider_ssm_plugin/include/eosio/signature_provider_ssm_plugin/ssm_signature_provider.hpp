 ...
 * at `eosio/signature_provider_ssm_plugin/ssm_signature_provider.hpp`.
 *
 * This implements the `SSM:<param-ref>` spec grammar -- a sibling of the built-in `KEY:` / `KEYD:` forms and the kms
 ...
 * `plugin = eosio::signature_provider_ssm_plugin` (channeld links the plugin; enablement is the operator's choice). A
 * host application that does not use the plugin can instead link this code and call
 *   plugin.register_spec_handler("SSM", &eosio::sigprov::ssm::create_ssm_provider);
 ...
 * `eosio::sigprov::aws::region_client_cache`). Construction is offline: no
 ...
 *
 * Delegates to `eosio::sigprov::aws::throw_aws_error`: a retryable error
 * (throttling, timeout, service-internal) throws
 * `eosio::chain::signing_transient_exception`; a permanent one
 * (`ParameterNotFound`, access denied, KMS decrypt denied) throws
 * `eosio::chain::plugin_config_exception` with an IAM remediation hint
 ...
 * @throws eosio::chain::signing_transient_exception on a retryable failure
 *         that outlasted the SDK's own retries
 * @throws eosio::chain::plugin_config_exception on a permanent failure
 ...
 * @throws eosio::chain::plugin_config_exception on malformed spec, fetch
 *         failure, non-SecureString parameter, empty / unparseable value, or
 *         pinned-pubkey mismatch
 * @throws eosio::chain::pending_impl_exception for `chain_key_type_sui`
 * @throws eosio::chain::signing_transient_exception if the fetcher classified
