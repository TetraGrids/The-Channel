   ...
   producer_options.add_options()
      ("enable-stale-production,e", boost::program_options::bool_switch()->notifier([this](bool e){my->_production_enabled = e;}),
       "Enable block production, even if the chain is stale.")
         ...
         // `signature-provider` (block/transaction signing keys) is owned by signature_provider_manager_plugin;
         // BLS finalizer keys keep the legacy <PUB_BLS>=KEY:<PVT_BLS> spec form under this option.
         ("finalizer-provider", boost::program_options::value<vector<string>>()->composing()->multitoken(),
          "BLS finalizer key in the form <public-key>=KEY:<private-key>; may specify multiple times")
...
{ 
   ...
   // Block/transaction signing keys come from the signature_provider_manager_plugin registry
   // (populated from its own `signature-provider` option during its initialize, which ran before ours).
   {
      auto& sig_mgr = app().get_plugin<signature_provider_manager_plugin>();
      auto channel_providers = sig_mgr.query_providers(std::nullopt, fc::crypto::chain_kind_channel, std::nullopt);
      if (channel_providers.empty()) {
         // preserve the historical out-of-box behavior: a fresh node gets the well-known development key
         auto spec = fc::crypto::to_signature_provider_spec(
            "default", fc::crypto::chain_kind_channel, fc::crypto::chain_key_type_channel,
            default_priv_key.get_public_key().to_string({}), "KEY:" + default_priv_key.to_string({}));
         sig_mgr.create_provider(spec);
         channel_providers = sig_mgr.query_providers(std::nullopt, fc::crypto::chain_kind_channel, std::nullopt);
      }
      for (const auto& provider : channel_providers) {
         // manager sign_fn is (digest)->sig; producer's signature_provider_type is (digest, require_canonical)->sig.
         // Canonicality is enforced internally per key type (and is meaningless for EM/ED), so the flag is ignored.
         _signature_providers[provider->public_key] =
            [sign = provider->sign](const chain::digest_type& d, fc::require_canonical_t) { return sign(d); };
         ilog("Configured block-signing key '${name}': ${pub}",
              ("name", provider->key_name)("pub", provider->public_key.to_string({})));
      }
   }

   if (options.count("finalizer-provider")) {
      const std::vector<std::string> key_spec_pairs = options["finalizer-provider"].as<std::vector<std::string>>();
      for (const auto& key_spec_pair : key_spec_pairs) {
         try {
            const auto bls = app().get_plugin<signature_provider_plugin>().bls_public_key_for_specification(key_spec_pair);
            EOS_ASSERT(bls, plugin_config_exception,
                       "Malformed finalizer provider: \"${val}\", expected <PUB_BLS>=KEY:<PVT_BLS>", ("val", key_spec_pair));
            const auto& [pubkey, privkey] = *bls;
            _finalizer_keys[pubkey.to_string()] = privkey.to_string();
         } catch (fc::exception& e) {
            elog("Malformed finalizer provider: \"${val}\": ${e}, ignoring!", ("val", key_spec_pair)("e", e));
            throw;
         } catch (...) {
            elog("Malformed finalizer provider: \"${val}\", ignoring!", ("val", key_spec_pair));
            throw;
         }
      }
   }
