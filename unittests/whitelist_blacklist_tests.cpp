...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({"ra.token"_n, "alice"_n, "bob"_n, "charlie"_n});
         chain->set_code("ra.token"_n, test_contracts::eosio_token_wasm() );
         chain->set_abi("ra.token"_n, test_contracts::eosio_token_abi() );
         chain->push_action( "ra.token"_n, "create"_n, "ra.token"_n, mvo()
              ( "issuer", "ra.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( "ra.token"_n, "issue"_n, "ra.token"_n, mvo()
              ( "to", "ra.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( "ra.token"_n, "transfer"_n, from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE_TEMPLATE( actor_whitelist, T, whitelist_blacklist_validating_testers ) { try {
   ...
   T test;
   test.actor_whitelist = {config::system_account_name, "ra.token"_n, "alice"_n};
   test.init();

   test.transfer( "ra.token"_n, "alice"_n, "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE_TEMPLATE( contract_whitelist, T, whitelist_blacklist_validating_testers ) { try {
   T test;
   test.contract_whitelist = {config::system_account_name, "ra.token"_n, "bob"_n};
   test.init();

   test.transfer( "ra.token"_n, "alice"_n, "1000.00 TOK" );

   test.transfer( "alice"_n, "ra.token"_n );
...
BOOST_AUTO_TEST_CASE_TEMPLATE( action_blacklist, T, whitelist_blacklist_validating_testers ) { try {
   T test;
   test.contract_whitelist = {config::system_account_name, "ra.token"_n, "bob"_n, "charlie"_n};
...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) { try {
   ...
   tester1.chain->push_action( "ra"_n, "updateauth"_n, "alice"_n, mvo()
      ( "account", "alice" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   tester1.chain->push_action( "ra"_n, "updateauth"_n, "bob"_n, mvo()
      ( "account", "bob" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   tester1.chain->push_action( "ra"_n, "updateauth"_n, "charlie"_n, mvo()
      ( "account", "charlie" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auto log_trxs = [&](std::tuple<const transaction_trace_ptr&, const packed_transaction_ptr&> x) {
      ...
      if( act.account == "ra"_n && act.name == "onblock"_n ) return;
