...
const std::string eosio_token = name("ra.token"_n).to_string();

static void create_accounts(validating_tester& chain) {
   chain.produce_block();
   chain.create_accounts({"ra.msig"_n, "ra.token"_n});
   chain.produce_block();
   ...
   chain.set_code("ra.token"_n, test_contracts::eosio_token_wasm());
   chain.set_abi("ra.token"_n, test_contracts::eosio_token_abi());
   chain.set_code("ra.msig"_n, test_contracts::eosio_msig_wasm());
   chain.set_abi("ra.msig"_n, test_contracts::eosio_msig_abi());
...
static void propose_approve_msig_trx(validating_tester& chain, const name& proposal_name, const permission_level& perm, const fc::variant& pretty_trx) {
   ...
   chain.push_action("ra.msig"_n, "propose"_n, vector<permission_level>{perm},
      mvo()
         ("proposer",      "tester")
   ...
   chain.push_action("ra.msig"_n, "approve"_n, vector<permission_level>{perm},
      mvo()
         ("proposer",      "tester")
         ("proposal_name", proposal_name)
         ("level",         perm)
...
static void exec_msig_trx(validating_tester& chain, name proposal_name, const vector<permission_level>& perm) {
   chain.push_action("ra.msig"_n, "exec"_n, perm,
      mvo()
         ("proposer",      "tester")
         ("proposal_name", proposal_name)
         ("executer",      "tester")
...
static asset get_currency_balance(const validating_tester& chain, account_name account) {
   return chain.get_currency_balance("ra.token"_n, symbol(SY(4,CUR)), account);
}
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.produce_block();
   chain.push_action("ra.token"_n, "create"_n, "ra.token"_n, mutable_variant_object()
      ("issuer", eosio_token)
      ("maximum_supply", "9000000.0000 CUR")
   );

   chain.push_action("ra.token"_n, name("issue"), "ra.token"_n, fc::mutable_variant_object()
           ("to",       eosio_token)
           ("quantity", "1000000.0000 CUR")
           ("memo", "for stuff")
   );

   auto trace = chain.push_action("ra.token"_n, name("transfer"), "ra.token"_n, fc::mutable_variant_object()
       ("from", eosio_token)
       ("to", "tester")
   ...
   auto liquid_balance = get_currency_balance(chain, "ra.token"_n);
   ...
   trace = chain.push_action("ra.token"_n, name("transfer"), "tester"_n, fc::mutable_variant_object()
       ("from", "tester")
       ("to", "tester2")
       ("quantity", "1.0000 CUR")
       ("memo", "hi" )
   ...
   liquid_balance = get_currency_balance(chain, "ra.token"_n);
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, "tester"_n);
...
BOOST_AUTO_TEST_CASE( canceldelay_test ) { try {
   ...
   chain.produce_block();
   chain.create_account("ra.token"_n);
   chain.produce_block();
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) { try {
...

   {
      ...
      trace = chain.push_action("ra.token"_n, name("transfer"), vector<permission_level>{{"tester"_n, "first"_n}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "1.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action("ra.token"_n, name("transfer"), vector<permission_level>{{"tester"_n, "second"_n}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "5.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action("ra.token"_n, name("transfer"), vector<permission_level>{{"tester"_n, config::owner_name}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "10.0000 CUR")
          ("memo", "hi" ),
