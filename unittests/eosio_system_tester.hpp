...
public:
   ...
   eosio_system_tester(Lambda setup) {
      ...
      T::create_accounts({ "ra.token"_n, "ra.ram"_n, "ra.ramfee"_n, "ra.stake"_n,
               "ra.bpay"_n, "ra.vpay"_n, "ra.saving"_n, "ra.names"_n, "ra.rex"_n });

      T::produce_block();

      T::set_code( "ra.token"_n, test_contracts::eosio_token_wasm() );
      T::set_abi( "ra.token"_n, test_contracts::eosio_token_abi() );

      {
         const auto& accnt = T::control->db().template get<account_object,by_name>( "ra.token"_n );
         abi_def abi;
         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
         token_abi_ser.set_abi(std::move(abi), abi_serializer::create_yield_function( T::abi_serializer_max_time ));
      }

      create_currency( "ra.token"_n, config::system_account_name, core_from_string("10000000000.0000") );
      issue(config::system_account_name,      core_from_string("1000000000.0000"));
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( name("ra") ) );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
            get_balance(name("ra")) + get_balance(name("ra.ramfee")) + get_balance(name("ra.stake")) + get_balance(name("ra.ram")) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = T::get_row_by_account( "ra.token"_n, act, "accounts"_n, name(symbol(CORE_SYMBOL).to_symbol_code().value) );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( "ra.token"_n, "issue"_n, manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( "ra.token"_n, "transfer"_n, manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   void issue_and_transfer( const name& to, const asset& amount, const name& manager = config::system_account_name ) {
      signed_transaction trx;
      trx.actions.emplace_back( T::get_action( "ra.token"_n, "issue"_n,
      ...
      if ( to != manager ) {
         trx.actions.emplace_back( T::get_action( "ra.token"_n, "transfer"_n,
                                               vector<permission_level>{{manager, config::active_name}},
                                               mutable_variant_object()
                                               ("from",     manager)
                                               ("to",       to )
   ...
   fc::variant get_stats( const string& symbolname ) {
      ...
      vector<char> data = T::get_row_by_account( "ra.token"_n, name(symbol_code), "stat"_n, name(symbol_code) );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( "ra.msig"_n, config::system_account_name );
         BOOST_REQUIRE_EQUAL( T::success(), buyram( name("ra"), name("ra.msig"), core_from_string("5000.0000") ) );
         T::produce_block();
         ...
         T::set_code( "ra.msig"_n, test_contracts::eosio_msig_wasm() );
         T::set_abi( "ra.msig"_n, test_contracts::eosio_msig_abi() );

         T::produce_block();
         const auto& accnt = T::control->db().template get<account_object,by_name>( "ra.msig"_n );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( name("ra"), name("alice1111111"), core_from_string("650000000.0000"), name("ra") );
