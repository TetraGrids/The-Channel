 ...
 * For each action, an example regarding how to use the action with the chan command line is given.
 *
 ...
 * the cases of combined use of key/value and first/second involving map,pair in the chan are documented here.
 * so handling of std::pair is NOT the same as the handling of a general struct such as struct mystruct!
 *
 * When assigning data input with chan:
