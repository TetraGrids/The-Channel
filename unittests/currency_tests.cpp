...
class currency_tester : public T {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return T::get_currency_balance("ra.token"_n, symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action("ra.token"_n, "issue"_n, mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      {
         T::create_account( "ra.token"_n);
         T::set_code( "ra.token"_n, test_contracts::eosio_token_wasm() );

         auto result = push_action("ra.token"_n, "create"_n, mutable_variant_object()
                 ("issuer",       eosio_token)
         ...
         result = push_action("ra.token"_n, "issue"_n, mutable_variant_object()
                 ("to",       eosio_token)
...
template <typename T>
const name currency_tester<T>::eosio_token = "ra.token"_n;
...
BOOST_AUTO_TEST_CASE_TEMPLATE( bootstrap, T, currency_testers ) try {
   ...
   T t;
   auto actual = t.get_currency_balance("ra.token"_n, expected.get_symbol(), "ra.token"_n);
...
BOOST_AUTO_TEST_CASE_TEMPLATE( test_transfer, T, currency_testers ) try {
   ...
   {
      auto trace = chain.push_action("ra.token"_n, "transfer"_n, mutable_variant_object()
...
BOOST_AUTO_TEST_CASE_TEMPLATE( test_duplicate_transfer, T, currency_testers ) {
   ...
   BOOST_REQUIRE_THROW(chain.push_action("ra.token"_n, "transfer"_n, mutable_variant_object()
                                    ("from", chain.eosio_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_proxy_deferred, pre_disable_deferred_trx_currency_tester ) try {
   ...
   {
      auto trace = push_action("ra.token"_n, "transfer"_n, mutable_variant_object()
         ("from", eosio_token)
         ("to",   "proxy")
         ("quantity", "5.0000 CUR")
         ("memo", "fund Proxy")
