...
public:
   ...
   eosio_token_tester() {
      ...
      T::create_accounts( { "alice"_n, "bob"_n, "carol"_n, "ra.token"_n } );
      T::produce_block();
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = eosio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = T::get_row_by_account( "ra.token"_n, acc, "accounts"_n, name(symbol_code) );
   ...
   T::action_result create( account_name issuer,
      ...
      return push_action( "ra.token"_n, "create"_n, mvo()
           ( "issuer", issuer)
...
BOOST_AUTO_TEST_SUITE(ra_token_tests)
