...
BOOST_AUTO_TEST_CASE_TEMPLATE( link_auths, TESTER, validating_testers ) { try {
   ...
   chain.link_authority(name("alice"), name("ra"), name("spending"), name("reqauth"));
   ...
   BOOST_CHECK_THROW( chain.link_authority(name("alice"), name("ra"), name("spending"), name("reqauth")), action_validate_exception);

   // Unlink alice with eosio reqauth
   chain.unlink_authority(name("alice"), name("ra"), name("reqauth"));
   ...
   chain.link_authority(name("alice"), name("ra"), name("scud"));
...
BOOST_AUTO_TEST_CASE_TEMPLATE( link_then_update_auth, TESTER, validating_testers ) { try {
   ...
   chain.link_authority(name("alice"), name("ra"), name("first"), name("reqauth"));
...
BOOST_AUTO_TEST_CASE_TEMPLATE( create_account, TESTER, validating_testers ) { try {
   ...
   chain.create_account(name("ra.test1"));

   // Creating account with eosio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account(name("ra.test2"), name("joe")), action_validate_exception,
...
BOOST_AUTO_TEST_CASE_TEMPLATE( any_auth, TESTER, validating_testers ) { try {
   ...
   chain.link_authority( name("alice"), name("ra"), name("ra.any"), name("reqauth") );
   chain.link_authority( name("bob"), name("ra"), name("ra.any"), name("reqauth") );
...
BOOST_AUTO_TEST_CASE_TEMPLATE( delete_auth, TESTER, validating_testers ) { try {
   ...
   trace = chain.push_action("ra.token"_n, name("transfer"), "ra.token"_n, fc::mutable_variant_object()
       ("from", "ra.token")
       ("to", "tester")
   ...
   auto liquid_balance = chain.get_currency_balance("ra.token"_n, symbol(SY(4,CUR)), "ra.token"_n);
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = chain.get_currency_balance("ra.token"_n, symbol(SY(4,CUR)), "tester"_n);
   ...
   liquid_balance = chain.get_currency_balance("ra.token"_n, symbol(SY(4,CUR)), "ra.token"_n);
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = chain.get_currency_balance("ra.token"_n, symbol(SY(4,CUR)), "tester"_n);
   BOOST_REQUIRE_EQUAL(asset::from_string("99.0000 CUR"), liquid_balance);
   liquid_balance = chain.get_currency_balance("ra.token"_n, symbol(SY(4,CUR)), "tester2"_n);
